...
})({"1abgq":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
function generate(tree, source) {
    ...
    let formatElement = "";
    if (kind === "list") formatElement = generateList(firstChild, source);
    else if (kind === "call") formatElement = generateCall(firstChild, source);
    else if (kind === "ident") formatElement = generateIdent(firstChild.text);
 ...
 */ function generateList(node, source) {
    ...
    for(let i = 0; i < childCount; i++){
        ...
        let content = "";
        let type = child.type;
        if (type === "ident") content = generateIdent(child.text);
        else if (type === "list") content = generateList(child, source);
        else if (type === "call") content = generateCall(child, source);
 ...
 */ function generateCall(node, source) {
    ...
    if (nodeContent === `Line(Soft)`) return "soft_line_break()";
    else if (nodeContent === `Line(Hard)`) return "hard_line_break()";
    else if (nodeContent === `Line(Empty)`) return "empty_line()";
...
},{}]},["1abgq","aR1JP"], "aR1JP", "parcelRequireea42")
